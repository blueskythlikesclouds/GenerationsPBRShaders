...
float4 main(in float2 texCoord : TEXCOORD) : COLOR
{
    ...
    color += tex2Dlod(s0, float4(texCoord * g_Param.z,                     0, g_Param.w)) * 0.29411764705882354;
    color += tex2Dlod(s0, float4(texCoord * g_Param.z + off1 * g_Param.xy, 0, g_Param.w)) * 0.35294117647058826;
    color += tex2Dlod(s0, float4(texCoord * g_Param.z - off1 * g_Param.xy, 0, g_Param.w)) * 0.35294117647058826;
    return color;
}
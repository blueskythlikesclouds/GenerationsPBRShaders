...
float ComputeMieScattering(float3 position)
{
    ...
    return (1.0 - g_G * g_G) / (4.0 * PI * pow(abs(1.0 + g_G * g_G + -2.0 * g_G * cosTheta), 1.5));
}


float4 main(in float4 unused : SV_POSITION, in float4 svPos : TEXCOORD0, in float4 texCoord : TEXCOORD1) : SV_TARGET
{
    float depth = g_DepthTexture.SampleLevel(g_PointClampSampler, texCoord.xy, 0);
    ...
    float3 viewPosition = GetPositionFromDepth(svPos.xy, depth, g_MtxInvProjection);

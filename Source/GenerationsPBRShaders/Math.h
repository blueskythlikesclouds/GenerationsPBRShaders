...
struct Frustum
{
    ...
    bool intersects(const Eigen::AlignedVector3f& point) const 
    {
        for (auto& plane : Planes)
        {
    ...
    bool intersects(const Eigen::AlignedVector3f& center, const float radius) const 
    {
        for (auto& plane : Planes)
        {
...
struct OBB
{
    ...
    Eigen::AlignedVector3f axis[3];
    Eigen::AlignedVector3f halfExtents;
    ...
    OBB(const Eigen::Matrix4f& matrix, const float bounds = 1.0f)
    {
        ...
        halfExtents = (Eigen::AlignedVector3f(matrix.col(0).norm(), matrix.col(1).norm(), matrix.col(2).norm()) / bounds) / 2.0f;
    }

    Eigen::AlignedVector3f closestPoint(const Eigen::AlignedVector3f& point) const
    {
        const Eigen::AlignedVector3f direction = point - center;
    ...
    float closestPointDistanceSquared(const Eigen::AlignedVector3f& point) const
    {
        return (point - closestPoint(point)).squaredNorm();
    }
};

inline Eigen::AlignedVector3f transform(const Eigen::AlignedVector3f& position, const Eigen::Matrix4f& matrix)
{
    return (matrix * Eigen::Vector4f(position.x(), position.y(), position.z(), 1)).head<3>();
}

inline Eigen::AlignedVector3f transformNormal(const Eigen::AlignedVector3f& normal, const Eigen::Matrix4f& matrix)
{
    return (matrix * Eigen::Vector4f(normal.x(), normal.y(), normal.z(), 0)).head<3>();
}

const Eigen::AlignedVector3f AABB_CORNERS[8] =
{
    { +1.0f, +1.0f, +1.0f },
    { +1.0f, +1.0f, -1.0f },
    { +1.0f, -1.0f, +1.0f },
